...
from {{ cookiecutter.module_name }}.services import (

)
from {{ cookiecutter.module_name }}.components.Fehlermeldung import Fehlermeldung
from {{ cookiecutter.module_name }}.styling import StandardStyling


if TYPE_CHECKING:
    from {{ cookiecutter.module_name }}.app import {{ cookiecutter.app_name|lower|replace('-', '_') }}
...
class MainStreamExampleComponent(toga.Box):
    ...
    def setup_input_fields(self, start_values):
        ...
        self.lawyer_name_input = toga.TextInput(
            value=start_values.get("name", ""), style=StandardStyling.standard_input_style()
        )
        self.lawyer_email_input = toga.TextInput(
            value=start_values.get("email", ""), style=StandardStyling.standard_input_style()
        )
        self.lawyer_phone_input = toga.TextInput(
            value=start_values.get("phone", ""), style=StandardStyling.standard_input_style()
        )
        self.lawyer_fax_input = toga.TextInput(
            value=start_values.get("fax", ""), style=StandardStyling.standard_input_style()
        )
    ...
    def generate_lawyer_list(self):
        ...
        start_values = lawyer_data.get("start_values", {})
        lawyer_list = []
        for lawyer_id, details in start_values.items():
            lawyer_list.append(
    ...
    def create_application(self, widget):
        ...
        daten_aus_formular = {
            ...
            "LAWYER_TITLE": self.start_values.get("title"),
            "LAWYER_SPECIALTY": self.start_values.get("specialty"),
            # "BEWEISMITTEL_LISTE": {
        ...
        if success:
            ...
            if self.antrag_manager_widget:
                self.footer_box.remove(self.antrag_manager_widget)
            ...
            self.antrag_manager_widget = AntragDateiManager(app=self.app, base_docx_datei=output_file_path)
            ...
            self.footer_box.add(self.antrag_manager_widget)
